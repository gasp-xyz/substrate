...
#[rpc(client, server)]
pub trait VestingMangataApi<
	BlockHash,
	AccountId,
	TokenId,
	Balance,
	BlockNumber,
	ResponseTypeVestingInfosWithLockedAt,
>
{
	#[method(name = "vesting_getVestingLockedAt")]
	fn get_vesting_locked_at(
		&self,
		who: AccountId,
		token_id: TokenId,
		at_block_number: Option<BlockNumber>,
		at: Option<BlockHash>,
	) -> RpcResult<ResponseTypeVestingInfosWithLockedAt>;
}
	...
	AccountId: Codec + MaybeDisplay + MaybeFromStr + sp_std::fmt::Debug,
{
	fn get_vesting_locked_at(
		...
		let runtime_api_result = api.get_vesting_locked_at(&at, who, token_id, at_block_number);
